...
public class ClientWorld extends World implements DataCache.CacheContext<ClientWorld> {
	...
	public ClientWorld(
		...
		this.setSpawnPoint(SpawnPoint.create(registryRef, new BlockPos(8, 64, 8), 0.0F, 0.0F));
	...
	private void addParticle(BlockPos pos, ParticleEffect parameters, VoxelShape shape, double y) {
		this.addParticle(
			pos.getX() + shape.getMin(Axis.X), pos.getX() + shape.getMax(Axis.X), pos.getZ() + shape.getMin(Axis.Z), pos.getZ() + shape.getMax(Axis.Z), y, parameters
		);
	}
	...
	private void addParticle(ParticleEffect particleEffect, boolean bl, boolean bl2, double d, double e, double f, double g, double h, double i) {
		try {
			...
			crashReportSection.add(
				"Parameters", () -> ParticleTypes.TYPE_CODEC.encodeStart(this.getRegistryManager().getOps(NbtOps.INSTANCE), particleEffect).toString()
			);
