...
public class WaxedIronBarsBlock extends PaneBlock {
    private final Block unwaxedVersion;

    public WaxedIronBarsBlock(Block unwaxedVersion, Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (stack.getItem() instanceof AxeItem) {
            if (world instanceof ServerWorld) {
                BlockState newState = unwaxedVersion.getStateWithProperties(state);
