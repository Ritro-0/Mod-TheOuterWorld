...
public class QuantumPadBlock extends BlockWithEntity {
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		...
		// Check severed state from persistent memory; severed pads cannot be lit or used
		ServerWorld serverWorld = (ServerWorld) world;
		MinecraftServer server = serverWorld.getServer();
		QuantumPadLinkState linkState = QuantumPadLinkState.get(server);
		boolean isSevered = linkState.isSevered(world.getRegistryKey(), pos);
		if (isSevered) {
			if (state.get(ACTIVATED)) {
				world.setBlockState(pos, state.with(ACTIVATED, false));
			}
			player.sendMessage(Text.literal("[QuantumPad] Link severed — pad is disabled."), false);
    ...
    private void teleportPlayer(PlayerEntity player, BlockPos pos, World world) {
		...
		// Respect severed link: prevent teleport if this pad is severed
		QuantumPadLinkState linkState = QuantumPadLinkState.get(server);
		if (linkState.isSevered(world.getRegistryKey(), pos)) {
			player.sendMessage(Text.literal("[QuantumPad] Link severed — teleportation disabled."), false);
			return;
		}
		...
		// Record link in persistent memory (allows one-way first use then creates link)
		linkState.linkPads(currentDimension, pos, targetDimension, targetPadPos, server);
	...
	public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
		if (!world.isClient()) {
			...
			QuantumPadLinkState.get(sw.getServer()).registerPad(world.getRegistryKey(), pos, sw.getServer());
		}
	...
	public void onStateReplaced(BlockState state, ServerWorld world, BlockPos pos, boolean moved) {
		if (!moved) {
			QuantumPadLinkState stateStore = QuantumPadLinkState.get(world.getServer());
			stateStore.severForPartnerOf(world.getRegistryKey(), pos, world.getServer());
			stateStore.unregisterPad(world.getRegistryKey(), pos, world.getServer());
		}
