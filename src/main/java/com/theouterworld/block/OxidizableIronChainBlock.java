...
public class OxidizableIronChainBlock extends ChainBlock implements Oxidizable {
    ...
    public OxidizableIronChainBlock(Oxidizable.OxidationLevel degradationLevel, Settings settings) {
        super(settings);
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        // DEBUG: Log randomTick calls (remove after verification)
        OuterWorldMod.LOGGER.info("[ChainDebug] Chain randomTick: {}, level={}, shouldOxidize={}", 
            pos, this.degradationLevel, OxidizableIronBehavior.shouldOxidize(world, pos));
    ...
    private void doOxidation(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        // DEBUG: Log oxidation attempts (remove after verification)
        OuterWorldMod.LOGGER.info("[ChainDebug] Chain doOxidation: {}, next={}, chance={}", 
            pos, nextState.map(s -> s.getBlock().toString()).orElse("NONE"), chance);
        
        if (nextState.isPresent()) {
            if (random.nextFloat() < chance) {
                world.setBlockState(pos, nextState.get());
                OuterWorldMod.LOGGER.info("[ChainDebug] Chain oxidized at {} to {}", pos, nextState.get().getBlock());
            }
        }
    }
